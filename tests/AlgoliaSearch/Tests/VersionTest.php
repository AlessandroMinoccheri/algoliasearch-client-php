...
class VersionTest extends AlgoliaSearchTestCase
{
    ...
    public function testVersionAddOnePrefixAndOneSuffix()
    {
        ...
        $this->assertRegExp('/^PHP \(\d+\.\d+\.\d+\); Algolia for PHP \(\d+\.\d+\.\d+\)$/', $userAgent);

        Version::addPrefixUserAgentSegment('Prefix integration', '0.0.8');
        Version::addSuffixUserAgentSegment('Suffix platform', '1.2.3');

        $userAgent = Version::getUserAgent();
        $this->assertEquals('Prefix integration (0.0.8); PHP ('.$this->php.'); Algolia for PHP ('.Version::VALUE.'); Suffix platform (1.2.3)', $userAgent);
    }
    ...
    public function testVersionAddTwoPrefixAndTwoSuffix()
    {
        ...
        $userAgent = Version::getUserAgent();
        $this->assertEquals('Another prefix (5.6.7); Prefix integration (0.0.8); PHP ('.$this->php.'); Algolia for PHP ('.Version::VALUE.'); Suffix platform (1.2.3); Different suffix (7.8.9)', $userAgent);
    ...
    public function testVersionDuplicatesPrefix()
    {
        ...
        $userAgent = Version::getUserAgent();
        $this->assertEquals('Another prefix (5.6.7); PHP ('.$this->php.'); Algolia for PHP ('.Version::VALUE.')', $userAgent);
    }
    ...
    public function testVersionDuplicatesSuffix()
    {
        ...
        $userAgent = Version::getUserAgent();
        $this->assertEquals('PHP ('.$this->php.'); Algolia for PHP ('.Version::VALUE.'); Another suffix (5.6.7)', $userAgent);
    }
    ...
    public function testVersionTwoPrefix()
    {
        ...
        $userAgent = Version::getUserAgent();
        $this->assertEquals('Another prefix (5.6.7); prefix (5.6.7); PHP ('.$this->php.'); Algolia for PHP ('.Version::VALUE.')', $userAgent);
    }
}
